...
save_model = False
range_value = "all"
num_tree_list = [500, 1000, 1500, 2000]
letter_list = ["C1-P"]
...
for letter in letter_list:
    ...
    writer = pd.ExcelWriter(os.path.join(main_folder, subfolder_output_results, 
                            str(datetime.datetime.today().date()) + " RF results - Counts %s 10 - %s.xlsx" %(range_value, letter)))
    ...
    for file in input_files:
        ...
        for num_tree in num_tree_list:
            ...
            for i in range(num_fold):
                ...
                print(datetime.datetime.now(), "\tRandom forest - Training %s \tkFold = %d" %(file, i+1))
                print("No of trees: %s" %num_tree)
    ...
    cols = ["Atoms","Euclidean distance","Filtration distance","Bin size","Bin num","Seed",
            "No trees","Min samples","MSE_train","MSE_CV","PCC_train","PCC_CV"]
...
plt.figure()
sns.scatterplot(x = y_pred, y = cv_y)

import matplotlib.pyplot as plt
import seaborn as sns
plt.figure()
sns.jointplot(x = y_pred, y = cv_y, kind = "hex") # kind = "hex", "kde"


import matplotlib.pyplot as plt
import seaborn as sns
plt.figure()
data = pd.DataFrame({"y_pred":y_pred, "cv_y":cv_y})
data.groupby(["y_pred", "cv_y"]).agg("count")
sns.heatmap(data)
"""