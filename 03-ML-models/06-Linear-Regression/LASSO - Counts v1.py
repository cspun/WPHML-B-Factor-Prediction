...
for letter in letter_list:
    ...
    writer = pd.ExcelWriter(os.path.join(main_folder, subfolder_output_results, 
                            str(datetime.datetime.today().date()) + " Lasso results - Counts %s 2 - %s.xlsx" %(range_value, letter)))
    ...
    input_files = [f for f in input_files if f.startswith("%s_10_5" %letter) or f.startswith("%s_15_7" %letter) or f.startswith("%s_20_10" %letter) 
                                            or f.startswith("%s_35_17" %letter) or f.startswith("%s_40_20" %letter) or f.startswith("%s_45_22" %letter) ]
    ...
    for file in input_files:
        ...
        for alpha in alpha_list:
            ...
            for i in range(num_fold):
                ...
                # Training
                clf = Lasso(alpha = alpha, random_state = seed, max_iter = 20000)
