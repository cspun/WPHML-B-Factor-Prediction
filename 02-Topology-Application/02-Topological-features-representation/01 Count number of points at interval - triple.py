...
# Basic information
atom_types_list = [["N","O","P"]]
max_dimension = 1 # Maximum betti number
float_range = np.linspace(.5, 0.6, 2)
bin_sizes = [0.15, 0.1, 0.05]

for atom_types in atom_types_list:

    for euclidean_distance in [45, 40]:
        ...
        common_atoms = [f for f in short_names[0] if f in short_names[1] and f in short_names[2] ]
        ...
        for filtration_distance in filtration_distance_1:
            ...
            for bin_size in bin_sizes:
                ...
                for common_atom in common_atoms:
                    print("TFR \tatom %s \tEuclidean %s \tFiltration %s \tbin: %s \tatom: %s"%(atom_type, euclidean_distance, filtration_distance, bin_size, common_atom))
                    ...
                    indexes = [short_names[0].index(common_atom), short_names[1].index(common_atom), short_names[2].index(common_atom)]
