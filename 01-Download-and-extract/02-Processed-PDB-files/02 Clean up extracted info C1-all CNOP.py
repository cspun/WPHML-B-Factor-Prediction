...
letter = "P" # Choices are all atoms or one of CNOP
...
# STEP 3
#example = ["Extract_pdb_1dk1_B.pdb"]
...
for old_filename in extraction_files:
	...
	if old_filename.split("_")[1] == "pdb":
		...
		for line in oldfile:
			...
			# If previous line and current line are different positions, save the previous line
			if line[23:27] != prev_line[23:27]:
				if prev_line[0] != "Y":
					newfile.write(prev_line)
				prev_line = line

			# If previous line and current line are of same position
			else:
				# If the atoms are different, save previous line
				if line[13:16] != prev_line[13:16]:
					if prev_line[0] != "Y":
						newfile.write(prev_line)
					prev_line = line

				# If the atoms are the same, compare the two occupancy factors 
				else:
